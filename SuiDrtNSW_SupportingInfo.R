...
######################
        ...
        interactionDrtAgeSexRuralModel2enhanced <- gam(deaths ~  s(mm, k=4, fx=T, bs = 'cp')			
...
######################
  ...
  select dthyy, sum(deaths) as sui, sum(sui_und) as sui_und
        ...
        # sensitivity model
        strt=Sys.time()
        interactionDrtAgeSexRuralModel2SuicidePlusUndetermined <- gam(sui_und ~  s(mm, k=4, fx=T, bs = 'cc')		
        + s(DrtMales10_29rural)
        + s(DrtMales30_49rural)
        + s(DrtMales50plusrural)	
        + s(DrtFemales10_29rural)
